...
dist_matrix = euclidean_distances(X.toarray())

# 使用层次聚类（修改linkage为'ward'，并确保使用欧氏距离）
hc = AgglomerativeClustering(n_clusters=n_groups, metric='euclidean', linkage='ward')
cluster_labels = hc.fit_predict(X.toarray())  # 注意：当使用欧氏距离时，直接传入特征矩阵而非距离矩阵
...
"""
    ...
    output_dir = f"/data/zxli/kp/mult-prompt-semeval-cos/mult-prompt-semeval_hierarchical{n_groups}"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    file_name = f"/data/zxli/kp/mult-prompt-semeval-cos/mult-prompt-semeval_hierarchical{n_groups}/{i+1}.txt"
