...
epsilon = 0.02 #0.001 0.01 0.02 0.05 0.1
print(epsilon)
grained_type = [
    '/WORK_OF_ART',
    '/EVENT',
...
multi_prompt_list=[]
for i in range(0,9):
    file_path = '/data/zxli/kp/appendix_ner/mult-prompt-grainedner-9/{}.txt'.format(i+1)
    print(file_path)
...
for i in range(0,len(grained_type)):
    file_path = '/data/zxli/kp/appendix_ner/binary-prompt/{}.txt'.format(i+1)
...
if __name__ == '__main__':
    ...
    for i in range(0, len(lines)):
        ...
        pred_label, precision, recall = last_result(last_judge, confidences, labels)
    ...
    with open('/data/zxli/kp/appendix_ner/pred/pred_labels.txt', 'w', encoding='utf-8') as file:
        for item in pred_labels:
            file.write("%s\n" % item) 

    with open('/data/zxli/kp/appendix_ner/pred/confidences-14B.txt', 'w', encoding='utf-8') as file:
        for item in Con:
            file.write("%s\n" % item) 
