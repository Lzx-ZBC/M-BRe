...
file_path2 = "/data/zxli/kp/a-qwen2.5-instruct/relation_pred_frame-class-qwen2.5-wiki-relation-group4-random.txt"
output_file_path = "/data/zxli/kp/a-qwen2.5-instruct/output_data_group4-random.txt"  # 输出文件路径
...
# ---------------------------------------------------------------------------------------------------------
# 打开文件并读取内容
file_path = "/data/zxli/kp/a-qwen2.5-instruct/output_data_group4-random.txt"
...
# 定义文件路径
file_path = "/data/zxli/kp/a-qwen2.5-instruct/output_data_group4-random.txt"
sample_file_path = "/data/zxli/kp/a-qwen2.5-instruct/sample_data_all_group4-random.txt"  # 输出样本文件路径
...
# # 定义文件路径
# file_path1 = "/data/zxli/kp/a-wiki-extract/samplebuild1.txt"
# file_path2 = "/data/zxli/kp/a-qwen2.5-instruct/relation_pred_frame-class-qwen2.5-wiki-relation-group7-random.txt"
# output_file_path = "/data/zxli/kp/a-qwen2.5-instruct/output_data_group7-random.txt"  # 输出文件路径
...
# # ---------------------------------------------------------------------------------------------------------
# # 打开文件并读取内容
# file_path = "/data/zxli/kp/a-qwen2.5-instruct/output_data_group7-random.txt"
# relation_counts = {}  # 用于存储每个relation类别的数量
# na_relations = []  # 用于存储所有NA类别的条目
...
# # 定义文件路径
# file_path = "/data/zxli/kp/a-qwen2.5-instruct/output_data_group7-random.txt"
# sample_file_path = "/data/zxli/kp/a-qwen2.5-instruct/sample_data_all_group7-random2.txt"  # 输出样本文件路径
...
# # 检查样本数量是否达到429，不足则用NA填充
# if len(sample_data_list) < 429:
#     remaining = 429 - len(sample_data_list)
#     # 从NA关系中随机选择remaining个
#     if "NA" in relation_data and len(relation_data["NA"]) >= remaining:
#         na_samples = random.sample(relation_data["NA"], remaining)
#         sample_data_list.extend(na_samples)
#         sample_relation_counts["NA"] = sample_relation_counts.get("NA", 0) + remaining
#     else:
#         # 如果没有足够的NA样本，创建新的NA数据
#         for _ in range(remaining):
#             # 创建一个基本的NA数据（这里需要根据你的实际数据结构调整）
#             na_data = {
#                 "relation": "NA",
#                 # 添加其他必要字段或使用默认值
#             }
#             sample_data_list.append(na_data)
#             sample_relation_counts["NA"] = sample_relation_counts.get("NA", 0) + 1

# # 将抽取的样本输出到sample_data.txt中
# with open(sample_file_path, 'w', encoding='utf-8') as sample_file:
#     for sample_data in sample_data_list[:429]:  # 确保只输出429行
#         json.dump(sample_data, sample_file, ensure_ascii=False)  # 以 JSON 格式写入文件
#         sample_file.write("\n")  # 每个数据占一行
...
# for relation, count in sample_relation_counts.items():
#     print(f"{relation}: {count}")
# print(f"总样本数量: {len(sample_data_list[:429])}")