...
file_path2 = "/data/zxli/kp/a-qwen2.5-instruct/semeval/qwen2.5-7B-wiki-relation-semeval-group5-random.txt"
output_file_path = "/data/zxli/kp/a-qwen2.5-instruct/semeval/output_data_group5_random.txt"  # 输出文件路径
...
# 解析 JSON 数据，并为每个数据添加 relation 键值
new_data_list = []
...
# -------------------------------------------------------------------------------------------------------
# 打开文件并读取内容
file_path = "/data/zxli/kp/a-qwen2.5-instruct/semeval/output_data_group5_random.txt"
...
with open(file_path, 'r', encoding='utf-8') as file:
    for line in file:
        ...
        if relation_value ==  "Other":
            ...
            relation_counts[ "Other"] = relation_counts.get( "Other", 0) + 1
        else:
...
# 计算非NA类别的平均数量（四舍五入）
non_na_total = sum(count for relation, count in relation_counts.items() if relation !=  "Other")
non_na_count = len([relation for relation in relation_counts if relation !=  "Other"])
if non_na_count > 0:
...
# 定义文件路径
file_path = "/data/zxli/kp/a-qwen2.5-instruct/semeval/output_data_group5_random.txt"
sample_file_path = "/data/zxli/kp/a-qwen2.5-instruct/semeval/sample_data_all_group5_random.txt"  # 输出样本文件路径
...
for relation, data_list in relation_data.items():
    if relation ==  "Other":
        # 对NA关系随机抽取非NA类别的平均数量的样本量
        sample_count = min(len(data_list), avg_non_na_count)
        ...
        # 非NA关系全都要
        sample_data_list.extend(data_list)
        sample_relation_counts[relation] = len(data_list)  # 统计样本中每个关系类别的数量
